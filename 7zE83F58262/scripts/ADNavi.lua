...
-- White 4×4 DXT1 DDS shipped with the mod (gui/white.dds).
-- Using a pre-built file instead of writing it at runtime so the mod works
-- identically from a folder and from a ZIP package (io.open cannot write
-- into a ZIP, but Giants VFS can read assets from it just fine).
local WHITE_DDS     = MOD_DIRECTORY .. "gui/white.dds"
local overlayFailed = false   -- set true once createImageOverlay fails
